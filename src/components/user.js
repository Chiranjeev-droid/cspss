...
class User extends React.Component {
  ...
  selectSlots = () => {
    ...
    if (endTime < startTime) {
      ...
      this.state.parkingSlots.map((slotRef, index) => {
        slotRef.Bookings
          ? Object.keys(slotRef.Bookings).map((slot) => {
              ...
              if (
                ...
                console.log(
                  "if",
                  slotRef.Bookings[slot].bookingDay,
                  slotRef.Bookings[slot].bookingMonth,
                  slotRef.Bookings[slot].bookingYear
                );
                if (slotRef.Bookings[slot].booked === "true") {
                  if (currentTime > startTime) {
                    ...
                    ((startTime >= slotRef.Bookings[slot].startTime &&
                      startTime <= slotRef.Bookings[slot].endTime) ||
                      (startTime <= slotRef.Bookings[slot].startTime &&
                        endTime >= slotRef.Bookings[slot].startTime)) &&
                    bookingDay === slotRef.Bookings[slot].bookingDay &&
                    bookingMonth === slotRef.Bookings[slot].bookingMonth &&
  ...
  bookSlots = (slotIndex) => {
    this.state.parkingSlots.map((slot, index) => {
      if (slotIndex === index && this.state.bgColor[index] === "green") {
        ...
        slotIndex === index &&
        this.state.bgColor[index] === "yellow"
      ) {
  ...
  render() {
    const actions = [
      ...
      <FlatButton label="Book" primary={true} onClick={this.sendBookedSlots} />,
    ];
    const actions2 = [
      <FlatButton
        label="Cancel"
    ...
    return (
      <div
        ...
        style={{ width: "100%", minHeight: 900, maxHeight: "100%" }}
      >
        <div
          style={{
            height: "40px",
        ...
        >
          <center>
            <p
              style={{ fontSize: "24px" }}
            >{`Welcome ${this.state.displayName} to the ParkEasy`}</p>
          </center>
        </div>
        <div>
          ...
          >
                    ...
                    if (this.state.bgColor[index] === "red") {
                      ...
                      return (
                        <RaisedButton
                          ...
                          style={{ margin: "5px" }}
                          key={index}
                          backgroundColor={this.state.bgColor[index]}
                ...
                <TableBody stripedRows={true} displayRowCheckbox={false}>
                  {this.props.parkingArea ? (
                    this.props.parkingArea.map((parking) => {
                      return parking.Slots.map((slotRef, index) => {
                        return slotRef.Bookings
                          ? Object.keys(slotRef.Bookings).map((slot) => {
                              if (
                                slotRef.Bookings[slot].userUid ===
                                this.state.userUid
                              ) {
                                let startTimeHours = new Date(
                                  slotRef.Bookings[slot].startTime
                                ).getHours();
                                let startTimeMinutes = new Date(
                                  slotRef.Bookings[slot].startTime
                                ).getMinutes();
                                let endTimeHours = new Date(
                                  slotRef.Bookings[slot].endTime
                                ).getHours();
                                let endTimeMinutes = new Date(
                                  slotRef.Bookings[slot].endTime
                                ).getMinutes();
                                return (
                                  <TableRow key={index}>
                                    ...
                                    >
                                      {slotRef.Bookings[slot].parkingArea}
                                    </TableRowColumn>
                                    <TableRowColumn
                                      style={{
                                        fontSize: "24",
                                    ...
                                    >
                                      {slotRef.Bookings[slot].parkingPlace}
                                    </TableRowColumn>
                                    <TableRowColumn
                                      style={{
                                        fontSize: "24",
                                    ...
                                    >
                                      {slotRef.Bookings[slot].bookingDay}/
                                      {slotRef.Bookings[slot].bookingMonth + 1}/
                                      {slotRef.Bookings[slot].bookingYear}
                                    </TableRowColumn>
                                    <TableRowColumn
                                      style={{
            ...
            <div style={{ opacity: "0.7" }}>
              <Paper
                zDepth={4}
                style={{ marginLeft: "30%", width: "40%", maxHeight: "200px" }}
              >
              ...
              <div style={{ marginTop: "40px" }}>
                ...
                {this.props.usersFeedbacks ? (
                  this.props.usersFeedbacks.map((feedback, index) => {
                      ...
                      return (
                        ...
                        >
                          <ListItem
                            className="feed"
                            key={index}
                            style={{ alignContent: "center", fontSize: "34px" }}
                            ...
                            nestedItems={[
                              <div>
                                {feedback.Reply ? (
                                  Object.keys(feedback.Reply).map(
                                    (reply, index) => {
                                      return feedback.Reply[reply]
                                        .replyFeedbackPushKey ===
                                        feedback.feedbackPushKey ? (
                                        <div>
                                          <div style={{ marginBottom: "5px" }}>
                                            ...
                                            >{`${feedback.Reply[reply].displayName}: `}</span>
